<template>
    ...
    <div class="col-md-6">
      <div class="card">
        <div class="card-body text-center">
          ...
          <h4>{{ $t('identity.unavailable') }}</h4>
          <p class="text-muted">{{ errorMsg }}</p>
          <router-link to="/dashboard" class="btn btn-primary"><i class="fas fa-arrow-left"></i> {{ $t('common.back') }}</router-link>
        </div>
      </div>
    </div>
  </div>
  ...
  <div v-else-if="cardData" class="row justify-content-center">
    <div class="col-md-8">
        ...
        <div class="card-body p-4" style="background: linear-gradient(135deg, #2c5f2d 0%, #97bc62 100%); color: white; border-radius: 10px; position: relative; overflow: hidden;">
          ...
          <div style="position: relative; z-index: 1;">
            <div class="text-center mb-3">
              <h4 class="mb-0"><i class="fas fa-id-card"></i> {{ $t('identity.republic') }}</h4>
              <p class="mb-0" style="font-size: 0.9rem;">{{ $t('identity.biometric') }}</p>
              <p class="mb-0" style="font-size: 0.8rem;">{{ $t('identity.hub') }}</p>
            </div>
            ...
            <div class="row">
              ...
              <div class="col-8">
                <div class="mb-4">
                  <strong style="font-size: 0.9rem; letter-spacing: 1px;">{{ $t('identity.fullName') }}:</strong><br>
                ...
                <div class="mt-5 pt-4" style="border-top: 1px solid rgba(255,255,255,0.3);">
                  <small style="font-size: 0.8rem; opacity: 0.9;">
                    <i class="fas fa-calendar-alt"></i> {{ $t('identity.issuedOn') }}: <strong>{{ cardData.dateEmission }}</strong>
                  </small><br>
                  <small style="font-size: 0.7rem; opacity: 0.8;">
                    <i class="fas fa-shield-alt"></i> {{ $t('identity.secureDoc') }}
                  </small>
                </div>
              </div>
            </div>
            <div class="text-center mt-3 pt-2" style="border-top: 1px solid rgba(255,255,255,0.3);">
              <small style="font-size: 0.7rem;">{{ $t('identity.validOnly') }}</small>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="card mt-4">
        <div class="card-body text-center">
          <button @click="printCard" class="btn btn-primary me-2"><i class="fas fa-print"></i> {{ $t('common.print') }}</button>
          <router-link to="/dashboard" class="btn btn-secondary"><i class="fas fa-arrow-left"></i> {{ $t('common.back') }}</router-link>
        </div>
      </div>
    </div>
  </div>
